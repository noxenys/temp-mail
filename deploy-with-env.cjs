...
async function main() {
  ...
  // 查找temp_email_db数据库
  const targetDb = databases.find(db => db.name === 'temp_email_db');
  
  if (!targetDb) {
    warn('未找到temp_email_db数据库，将自动创建...');
    
    try {
      // 创建数据库
      execSync('npx wrangler d1 create temp_email_db', { stdio: 'inherit' });
      success('数据库创建成功');
      
      // 重新获取数据库列表
      d1ListOutput = execSync('npx wrangler d1 list --json', { encoding: 'utf8' });
      databases = JSON.parse(d1ListOutput);
      const newDb = databases.find(db => db.name === 'temp_email_db');
  ...
  info('验证部署结果...');
  try {
    execSync('npx wrangler d1 execute temp_email_db --command="PRAGMA table_info(messages);"', { stdio: 'ignore' });
    success('数据库连接验证成功');
  } catch (e) {
